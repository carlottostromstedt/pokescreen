...
class Inky:
    ...
    def set_image_at_position(self, image, x_offset=0, y_offset=0, saturation=0.5):
        """
        Copy an image to the display at a specific position.

        :param image: PIL image to copy
        :param x_offset: Horizontal offset from the left of the display
        :param y_offset: Vertical offset from the top of the display
        :param saturation: Saturation for quantization palette
        """
        # Validate image fits within display boundaries
        if x_offset + image.width > self.width or y_offset + image.height > self.height:
            raise ValueError(f"Image exceeds display boundaries. Max position: ({self.width}, {self.height}) trying to place at ({x_offset},{y_offset}) with image size {image.width}x{image.height}")

        # Convert image to paletted mode
        if not image.mode == "P":
            palette = self._palette_blend(saturation)
            palette_image = Image.new("P", (1, 1))
            palette_image.putpalette(palette + [0, 0, 0] * 248)
        ...
        # Copy image into display buffer at specified position
        for y in range(image.height):
            for x in range(image.width):
                self.buf[y_offset + y, x_offset + x] = image_array[y, x]

    def set_image_at_position2(self, image, width=300, height=400, x_offset=0, y_offset=0, saturation=0.5):
        """
        Resize and copy an image to the display at a specific position.
        ...
        # Validate resized image fits within display boundaries
        if x_offset + width > self.width or y_offset + height > self.height:
            raise ValueError(f"Resized image exceeds display boundaries. Max position: ({self.width}, {self.height})")

        # Convert image to paletted mode
        if not resized_image.mode == "P":
            palette = self._palette_blend(saturation)
            palette_image = Image.new("P", (1, 1))
            palette_image.putpalette(palette + [0, 0, 0] * 248)
            resized_image.load()
            resized_image = resized_image.im.convert("P", True, palette_image.im)

        # Convert image to NumPy array and reshape if necessary
        image_array = numpy.array(resized_image, dtype=numpy.uint8)
    ...
    def place_image(self, image, x, y, resize=True, saturation=0.5):
        """
        Place a smaller image onto the display at a specified x, y coordinate.

        :param image: PIL image to place
        :param x: x-coordinate of the top-left corner of the image
        :param y: y-coordinate of the top-left corner of the image
        :param resize: If True, resize image to fit display, maintaining aspect ratio
        :param saturation: Saturation for quantization palette - higher value results in a more saturated image
        """
        ...
        print(f"Placement Coordinates: ({x},{y})")
        ...
        # Adjust x and y to center the image if it's smaller than the display
        ...
        if x < 0 or y < 0 or x + image.width > self.width or y + image.height > self.height:
            raise ValueError(f"Image placement out of display bounds. Display is {self.width}x{self.height}, trying to place at ({x},{y}) with image size {image.width}x{image.height}")

        # First, fill the entire buffer with white
        self.buf.fill(WHITE)

        # Convert image to the display's color palette if not already in palette mode
        if not image.mode == "P":
            palette = self._palette_blend(saturation)
            palette_image = Image.new("P", (1, 1))
            palette_image.putpalette(palette + [0, 0, 0] * 248)
            ...
            image = image.convert("P", palette=palette_image)

        # Convert image to numpy array
        image_array = numpy.array(image, dtype=numpy.uint8)

        # Place the image at the specified coordinates
        self.buf[y:y+image_array.shape[0], x:x+image_array.shape[1]] = image_array

        print(f"Image placed at: ({x},{y})")
