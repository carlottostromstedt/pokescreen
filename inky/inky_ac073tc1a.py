...
class Inky:
    ...
    def draw_sub_image(self, sub_image, x_offset=0, y_offset=0, align_center=False):
        """
        Draws a smaller image inside the larger display canvas.

        :param sub_image: PIL.Image object to render.
        :param x_offset: Horizontal offset for the image position (default: 0).
        :param y_offset: Vertical offset for the image position (default: 0).
        :param align_center: If True, the image will be centered on the display, overriding offsets.
        """
        if not isinstance(sub_image, Image.Image):
            raise TypeError("sub_image must be a PIL.Image instance.")

        # Ensure the image dimensions are within the display size
        if sub_image.width > self.WIDTH or sub_image.height > self.HEIGHT:
            raise ValueError("Sub-image dimensions exceed the display size.")

        # Create a blank canvas for the display
        canvas = Image.new("P", (self.WIDTH, self.HEIGHT), self.WHITE)

        # Calculate centered position if align_center is True
        if align_center:
            x_offset = (self.WIDTH - sub_image.width) // 2
            y_offset = (self.HEIGHT - sub_image.height) // 2

        # Paste the sub-image onto the canvas
        canvas.paste(sub_image, (x_offset, y_offset))

        # Update the display buffer
        self._update_buffer_from_image(canvas)

    def _update_buffer_from_image(self, image):
        """
        Converts a PIL.Image to the internal buffer format and updates the display buffer.

        :param image: PIL.Image instance matching the display dimensions.
        """
        if image.size != (self.WIDTH, self.HEIGHT):
            raise ValueError("Image dimensions must match the display resolution.")

        # Convert image to 1-byte-per-pixel buffer using a palette
        palette_image = image.convert("P")
        self.buf = numpy.array(palette_image)

    def show(self):
        """
        Show the updated buffer on the e-Ink display.
        """
        self._update(self.buf.flatten())
